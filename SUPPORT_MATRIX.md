...
| Sampled value functions `$rose`, `$fell`, `$stable`, `$changed`, `$past(sig,N)` | Scalar sampled-value subset; `$past` finite depth | Bounded evidence | present: `tests/sv_fixtures/rose.sv`, `fell.sv`, `stable.sv`, `changed.sv`, `past.sv`; `tests/test_sv_fixture_e2e.py#test_e2e_rose` et al.; JSON fixtures retained | present: `tests/test_ast_importer.py`; `tests/test_formal_kinduction.py` | present: `tests/simulation/test_sim_rose.py`; `test_sim_fell.py`; `test_sim_stable.py`; `test_sim_past.py` | historical: CI run `28931676000`; current-commit rerun pending | present: simulator oracle and independent formal references | present: `tests/test_formal_sva_equiv.py`; k-induction for `$rose`, `$fell`, `$stable`, `$changed` in `tests/test_formal_kinduction.py`; Phase 10 `$rose` full-contract BMC depth 12 | present: local Yosys smoke `tests/test_synthesis_gates.py`; pending-remote: current-commit Verilator lint | present: `tests/test_nyquist_gaps.py#test_nyq22_past_depth_warning` | Four edge/state functions are k-induction proven; `$past` is BMC-only and promotion waits for current-commit remote gates. |
| `disable iff` | Disable condition gates attempts and clears active state | Bounded evidence | present: `tests/sv_fixtures/disable_iff.sv`; `tests/test_sv_fixture_e2e.py#test_e2e_disable_iff`; JSON fixture `tests/fixtures/disable_iff.json` (slang v11 `DisableIff` kind now handled by importer) | present: importer/composer coverage via tests and templates | present: `tests/simulation/test_sim_disable_iff.py` | historical: CI run `28931676000`; current-commit rerun pending | present: simulator oracle with reset/disable semantics | present: `tests/test_formal_sva_equiv.py`; Phase 10 arbitrary-disable pass/fail BMC depth 15 in `TestDisableIffSvaEquiv` | present: local Yosys smoke `tests/test_synthesis_gates.py`; pending-remote: current-commit Verilator lint | N/A: accepted control form | Full-contract bundle remains future work; promotion also waits for current-commit remote gates. |
| Named sequences | Non-circular named sequence references; circular references rejected | Bounded evidence | present: `tests/sv_fixtures/named_seq.sv`; `tests/test_sv_fixture_e2e.py#test_e2e_named_seq`; JSON fixtures retained (slang v11 `AssertionInstance` kind now handled) | present: `tests/test_named_sequences.py`; importer coverage | present: `tests/simulation/test_sim_named_seq.py --simulator=iverilog` | pending-remote: current-commit Verilator rerun required | present: source-authored `a ##1 b` reference and simulator oracle | present: `tests/test_formal_sva_equiv.py#TestNamedSequenceSvaEquiv`, independent pass/fail BMC depth 18 | present: local Yosys smoke `tests/test_synthesis_gates.py`; pending-remote: generated Verilator lint gate `tests/test_generated_lint.py` is configured in CI | present: `tests/test_named_sequences.py#test_circular_ref_rejected_with_sva_e003` | Dedicated non-circular BMC is now present; row remains bounded pending current-commit remote Verilator evidence. |
...
| Multi-clock path-one split/synchronize forms | Allowed `##1` clock changes and non-overlap cross-clock implication through trusted 2-DFF level synchronizer; emission requires explicit experimental opt-in | Trusted boundary | missing: no public real `.sv` fixture in root fixture corpus | present: `tests/test_multiclock.py`; default rejection and explicit opt-in are enforced, along with standard top contract and disable clearing | missing: no dynamic asynchronous clock-ratio or pulse-loss evidence | pending-remote: CI if dynamic tests are added later | trusted-boundary: per-domain semantics only; narrow source tokens can be missed or coalesced | trusted-boundary: full CDC/metastability and event-delivery proof excluded | trusted-boundary: local Yosys smoke and Verilator lint accept generated synchronizer structure under explicit opt-in; future handshake/toggle protocol and FPGA/prototype evidence remain planned | present: default emission rejection; unsupported cross-clock forms remain rejected | Default fail-closed. `--experimental-multiclock` exposes a prototype only; it is not an acknowledged pulse-transfer protocol or CDC sign-off. |
