...
The final v2.0 documentation-qualified baseline is
`d7ffe10f9294424482dd7a869a2867d3aee61e6e` on 2026-08-04:

- CI run [`30910167848`](https://github.com/VeriSymbolic-AI/sva2verilog/actions/runs/30910167848)
  passed all 13 jobs.
- Differential nightly run [`30910169662`](https://github.com/VeriSymbolic-AI/sva2verilog/actions/runs/30910169662)
  passed all three jobs.
- Full Formal run [`30910169857`](https://github.com/VeriSymbolic-AI/sva2verilog/actions/runs/30910169857)
  passed all eight shards.

The 2026-08-07 trust-hardening executable is
`5ad7e2f04c65348a709728447c64ff46acea1986`:

- CI run [`31167742453`](https://github.com/VeriSymbolic-AI/sva2verilog/actions/runs/31167742453)
  passed all 13 jobs.
- Differential nightly run [`31167748554`](https://github.com/VeriSymbolic-AI/sva2verilog/actions/runs/31167748554)
  passed all three jobs.
- Full Formal run [`31167747619`](https://github.com/VeriSymbolic-AI/sva2verilog/actions/runs/31167747619)
  passed all eight shards.

This exact-SHA qualification covers the machine-readable focused ledger,
real-project formal snapshots, tool-identity replay binding, pinned Linux live
runtime, and one bounded OpenTitan RTL slice. The slice killed a reviewed
latency mutant and exposed/fixed leading-delay import plus custom-reset bind
defects. These are stronger bounded evidence links, not grounds to promote a
row without its remaining row-specific evidence.
...
present for baseline executable `5ad7e2f`. The six strongest rows (`##N`, `[*N]`, sampled
...
| `##N` fixed delay | Positive fixed cycle delay, including a slang-v11 leading delay in an implication consequent | Bounded evidence | present: `tests/sv_fixtures/delay_fixed_1.sv`; `tests/sv_fixtures/delay_fixed_3.sv`; `tests/sv_fixtures/delay_fixed_8.sv`; `tests/fixtures/delay_assert.sv`; external slice property `tests/external_corpus/opentitan/two_cycle_property.sv` | present: `tests/test_pipeline_e2e.py#test_e2e_delay_assert_rejected`; `tests/test_sequential.py`; leading-delay regressions in `tests/test_ast_importer_mutation_boundaries.py` | present: `tests/simulation/test_sim_delay.py` | qualified at executable baseline `5ad7e2f` by CI run `31167742453` | present: `tests/simulation/test_sim_delay.py`; independent formal reference in `tests/test_formal_sva_equiv.py` | present: `tests/test_formal_sva_equiv.py#TestDelaySvaEquiv`; Phase 10 `##1` k-induction; real OpenTitan slice proves and a one-flop latency mutant fails in `tests/test_external_rtl_corpus.py`; Full Formal run `31167747619` | present: local Yosys smoke `tests/test_synthesis_gates.py`; generated RTL lint in CI run `31167742453` | N/A: accepted finite-state form | Strong bounded evidence. Exact-SHA workflow qualification is closed; an independent SVA-frontend comparison for the leading-delay shape remains before promotion. |
| `##[M:N]` bounded delay | Finite range with `M <= N`; lower bound 0 has `##0` caveat below | Bounded evidence | present: `tests/sv_fixtures/delay_range_0_1.sv`; `tests/sv_fixtures/delay_range_2_5.sv`; `tests/sv_fixtures/delay_range_0_15.sv`; issue-1 source shape `tests/sv_fixtures/imp_overlap_delay_range.sv` | present: `tests/test_ast_importer.py`; `tests/test_sequential.py`; real-source route `tests/test_sv_fixture_e2e.py#test_e2e_implication_ranged_delay_lower_bound`; compact-window unit contracts in `tests/test_implication_delay_window.py` | present locally: `tests/simulation/test_sim_delay.py`; `tests/simulation/test_sim_nfa_semantic_regressions.py#test_ranged_delay_enforces_every_window_boundary` checks +1/+2/+3 pass and +4 fail | present locally: the same +1/+2/+3/+4 boundary test under Verilator; exact-SHA remote qualification pending until Task 3 | present: `tests/simulation/test_sim_delay.py`; independent boundary vectors and shift/history reference | present locally: `tests/test_formal_sva_equiv.py#TestDelaySvaEquiv`; two parametrizations of `tests/test_implication_delay_window_formal.py#test_delay_window_full_contract_complete_proof` cover delay minima 1 and 2 plus pass/fail/overlap; the 16-pass/zero-skip Full Formal `nfa-composition` shard is pending exact-SHA qualification | present: local Yosys smoke; compact resource regression `tests/test_synthesis_gates.py#test_bounded_delay_implication_area_does_not_regress_to_threaded_nfa` | present: `tests/test_nyquist_gaps.py#test_nyq10_range_delay_min_gt_max_raises` | Support maturity remains bounded; the issue-specific local chain is present, but its frozen-SHA remote CI, differential-nightly, and Full Formal evidence is pending Task 3. |
...
| <code>\|-&gt;</code> overlapping implication | Single-cycle and fixed/ranged-delay or NFA-liftable consequent subset | Bounded evidence | present: `tests/sv_fixtures/impl_overlap_simple.sv`; `tests/sv_fixtures/impl_overlap_delay.sv`; issue-1 source fixture `tests/sv_fixtures/imp_overlap_delay_range.sv` | present: `tests/test_v151_p2_implication_nfa.py`; `tests/test_sequential.py`; `tests/test_sv_fixture_e2e.py#test_e2e_implication_ranged_delay_lower_bound`; compact backend/resource contracts in `tests/test_implication_delay_window.py` | present locally: implication suites plus `tests/simulation/test_sim_nfa_semantic_regressions.py#test_ranged_delay_enforces_every_window_boundary` for +1/+2/+3 pass and +4 fail | present locally: the same issue boundary test under Verilator; exact-SHA remote qualification pending until Task 3 | present: source reference compares the full checker contract and external disable; deterministic overlap vectors cover continuous starts | present locally: implication miters plus the two-parametrization independent full-monitor proof `tests/test_implication_delay_window_formal.py#test_delay_window_full_contract_complete_proof`, with pass/fail/overlap covers; the 16-pass/zero-skip Full Formal `nfa-composition` shard is pending exact-SHA qualification | Yosys 0.66: 19 generic cells in the reporter-compatible `proc; opt; stat` view versus 25 generic cells in the repository regression's later post-techmap view; the pipelines differ, and `tests/test_synthesis_gates.py#test_bounded_delay_implication_area_does_not_regress_to_threaded_nfa` preserves the `<=40` ceiling | present: `tests/test_v151_p2_implication_nfa.py#TestP2ImplicationNfaRejections` | The 19/25 generic-cell statistics are not target-library PPA or industrial evidence. Support maturity remains bounded; frozen-SHA CI, differential-nightly, and Full Formal qualification is pending Task 3. |
...
| `tests/external_corpus/opentitan/two_cycle_property.sv` | Leading `##2` implication consequent over external RTL | Real property source paired with an unmodified OpenTitan `prim_flop_2sync.sv` slice; see its provenance and non-CDC claim boundary. |
