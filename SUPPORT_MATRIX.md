...
| Multi-clock path-one split/synchronize forms | Allowed `##1` clock changes and non-overlap cross-clock implication through trusted 2-DFF level synchronizer | Trusted boundary | missing: no public real `.sv` fixture in root fixture corpus | present: `tests/test_multiclock.py`; standard top contract and disable clearing are structurally enforced | missing: no dynamic asynchronous clock-ratio or pulse-loss evidence | pending-remote: CI if dynamic tests are added later | trusted-boundary: per-domain semantics only; narrow source tokens can be missed or coalesced | trusted-boundary: full CDC/metastability and event-delivery proof excluded | trusted-boundary: local Yosys smoke and Verilator lint accept generated synchronizer structure; future handshake/toggle protocol and FPGA/prototype evidence remain planned | present: `tests/test_multiclock.py#test_overlapping_implication_cross_clock_rejected`; `#test_cross_clock_delay_n_neq_1_rejected_by_slang` | Top `start/disable_i/attempt_fired/disabled_o` is fixed, but a 2-DFF level synchronizer is not an acknowledged pulse-transfer protocol. Keep as trusted/prototype boundary. |
