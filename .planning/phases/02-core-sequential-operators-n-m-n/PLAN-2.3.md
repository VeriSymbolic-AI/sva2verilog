...
# Plan 2.3: Integration Tests, Golden File Harness, Concurrent-Attempt Stress Tests, and Boundary Tests

## Goal

Deliver comprehensive testing that proves Phase 2 operators work correctly end-to-end: golden file integration tests for deterministic codegen (TEST-02), concurrent-attempt stress tests that verify the bit-vector handles overlapping threads without false negatives/positives (TEST-05), and boundary tests that verify exact cycle behavior at N-1, N, M, M+1 boundaries (TEST-06). This plan also verifies `attempt_fired` and `overflow_flag` debug outputs (OUT-06) are exercised in every test scenario.
...
Proof: Byte-for-byte golden match; stress test assertions on overflow_flag; boundary assertions on pass/fail cycle timing
...
<task id="2.3.3">
<title>Concurrent-attempt stress tests for bit-vector implication</title>
<read_first>
...
1. `test_bv_width_sufficient_for_max_concurrent()`:
   - For `a |-> ##[2:5] b`: bv_width should be >= 5 (max 5 concurrent threads if antecedent fires every cycle)
   - Compile fixture, assert int(params["bv_width"]) >= consequent max length

2. `test_overflow_flag_not_set_within_capacity()`:
   - Construct a scenario: BV_WIDTH=5, antecedent fires 5 consecutive cycles
   - Verify structural correctness: the generated RTL has BV_WIDTH parameter >= 5
...
4. `test_concurrent_threads_structural_capacity()`:
   - For `a |-> ##8 b`: bv_width should be >= 8
   - For `a |-> ##1 b`: bv_width should be >= 1
   - For `a |-> ##[0:15] b`: bv_width should be >= 15
...
<acceptance_criteria>
- Test `test_bv_width_sufficient_for_max_concurrent` passes: bv_width >= max consequent delay
- Test `test_overflow_flag_structure_present` passes: emitted RTL contains overflow detection logic
- Test `test_concurrent_threads_structural_capacity` parametrized over [1, 5, 8, 15] verifies bv_width >= each value
...
3. `test_delay_zero_special_case()`:
   ...
   - Verify emitted RTL or composer behavior for zero delay
...
- Test `test_delay_zero_special_case` handles ##0 correctly (no crash, reasonable output)
- Test `test_delay_range_window_width` verifies delay_min and delay_max params for range delays
- Test `test_bv_width_boundary_for_implication` verifies bv_width >= consequent length
...
<task id="2.3.5">
<title>Full regression and integration validation</title>
<read_first>
...
4. Run full test suite command validation:
...
- Total test count increases from 131 (Phase 1) to at least 165 (Phase 2 adds 34+ tests)
</acceptance_criteria>
</task>
...
| Stress tests only verify structure, not runtime behavior | Medium | Acknowledged limitation. Phase 3 adds Icarus Verilog simulation oracle. Phase 2 stress tests verify structural capacity (bv_width, overflow logic presence). |
| Boundary test misses an edge case (off-by-one) | Medium | Parametrized tests cover powers-of-2 boundaries (1,2,4,8,16) and adjacent values. Combined with Phase 3 simulation. |
...
- [ ] Concurrent-attempt stress tests verify bv_width >= max consequent length (TEST-05)
