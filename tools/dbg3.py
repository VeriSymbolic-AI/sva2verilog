#!/usr/bin/env python3
"""Debug: add $display to child's always_ff"""
...
# Add $display to child's else branch
for k, sv in list(mods.items()):
    if k == checker.module_name:
        continue
    sv = sv.replace(
        'end else begin\n            active_q <= start;',
        'end else begin\n            $display("%0t '+k+': rst_n=%b start=%b a=%b", $time, rst_n, start, a);\n            active_q <= start;'
    )
    mods[k] = sv
...
module tb;
  ...
  initial begin
    $monitor("%0t PAR: act=%b pass=%b", $time, act, p);
