#!/usr/bin/env python3
"""Test: verify fix - change logic X=expr to wire X=expr or explicit assign"""
...
# FIX: change all intermediate comb. signals from logic to wire
# AND add explicit assign for combinational logic
sv = mods[checker.module_name]
# Keep 'logic' for registered signals, use 'wire' for everything else driven by children
replacements = [
    ('logic left_active, left_pass, left_fail;', 'wire left_active, left_pass, left_fail;'),
    ('logic right_active, right_pass, right_fail;', 'wire right_active, right_pass, right_fail;'),
    ('logic left_disabled, right_disabled;', 'wire left_disabled, right_disabled;'),
    # Fix combinational _body_* - must use wire, not logic
    ('logic _body_active =', 'wire _body_active ='),
    ('logic _body_pass   =', 'wire _body_pass   ='),
    ('logic _body_fail   =', 'wire _body_fail   ='),
]
