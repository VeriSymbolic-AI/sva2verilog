#!/usr/bin/env python3
"""Debug: check if disabling disable_i gating in bool_expr fixes x"""
...
# Replace output gating in children to skip disable_i check
for k, sv in list(mods.items()):
    if k == checker.module_name:
        continue
    # In bool_expr, replace: assign active = disable_i ? 1'b0 : active_q
    # with: assign active = active_q; (remove disable_i gating test)
    sv = sv.replace(
        'assign active        = disable_i ? 1\'b0 : active_q;',
        'assign active        = active_q;  // DEBUG: removed disable_i gate'
    )
    sv = sv.replace(
        'assign pass          = disable_i ? 1\'b0 : pass_q;',
        'assign pass          = pass_q;    // DEBUG: removed disable_i gate'
    )
    sv = sv.replace(
        'assign fail          = disable_i ? 1\'b0 : fail_q;',
        'assign fail          = fail_q;    // DEBUG: removed disable_i gate'
    )
    mods[k] = sv
