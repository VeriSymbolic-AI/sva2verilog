...
The local candidate is suitable for an anonymous Git commit. The thirteen reviewed
...
| P24-F13 | Medium | The first exact-SHA CI run passed all Icarus tests but failed its stale skip budget, while matrix setup installed Python 3.13 but silently reused the repository's Python 3.12 environment | The Icarus gate now permits only enumerated full-formal toolchain skip reasons under an exact 228 cap and raises its pass floor to 1500; the complete matrix job pins `UV_PYTHON`, dependency sync explicitly selects `${{ matrix.python }}`, and a workflow regression locks both contracts | Ordinary CI intentionally records formal-toolchain skips; the separate Full Formal workflow must still execute those tests with zero toolchain skips |
